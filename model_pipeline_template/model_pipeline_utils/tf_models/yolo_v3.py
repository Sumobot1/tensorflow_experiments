...
class YOLOV3:
    ...
    def make_last_layers(self, input_layer, num_filters, num_outputs, output_identifier):
        x = self.darknet_conv2d_bn_leaky(input_layer, num_filters, (1, 1),
                                         layer_num="x_{}_output_layer_{}".format(output_identifier, 0))
        x = self.darknet_conv2d_bn_leaky(x, num_filters * 2, (3, 3),
                                         layer_num="x_{}_output_layer_{}".format(output_identifier, 1))
        x = self.darknet_conv2d_bn_leaky(x, num_filters, (1, 1),
                                         layer_num="x_{}_output_layer_{}".format(output_identifier, 2))
        x = self.darknet_conv2d_bn_leaky(x, num_filters * 2, (3, 3),
                                         layer_num="x_{}_output_layer_{}".format(output_identifier, 3))
        x = self.darknet_conv2d_bn_leaky(x, num_filters, (1, 1),
                                         layer_num="x_{}_output_layer_{}".format(output_identifier, 4))

        y = self.darknet_conv2d_bn_leaky(x, num_filters * 2, (3, 3),
                                         layer_num="y_{}_output_layer_{}".format(output_identifier, 0))
        y = self.darknet_conv2d(y, num_outputs, (1, 1),
                                layer_name="y_{}_output_layer_{}".format(output_identifier, 1))
        return x, y
